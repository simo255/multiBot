...
3. Check `grok-agent` and `agy` (`agy --version`)
4. Confirm `agent status` or note user must `agent login` / set `CURSOR_API_KEY`
5. Antigravity: install via `curl -fsSL https://antigravity.google/cli/install.sh | bash` if missing; Google sign-in on first `agy`
6. Write memory: multiBot creates child bots via CreateAgent; children use CLI underground; read skill create-bot
7. Optional: clone this repo to `/workspace/multiBot` for template files
